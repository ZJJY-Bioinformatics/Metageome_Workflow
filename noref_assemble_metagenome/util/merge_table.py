...
def merge(aaastrIn, ostm, column = "relative_abundance"):
    """
    ...
    for f in aaastrIn:
        ...
        #if len(headers) == 5 or len(headers) == 4:
        names = headers[-1].split('#')[1].strip().split('\t')
        #else:
        #    print('merge_metaphlan_tables: wrong header format for "{}", please check your profiles.\n'.format(f))
        #    return
        # print(len(listmpaVersion))
        # if len(listmpaVersion) > 1:
        #     print('merge_tables: profiles from differrent versions of MetaPhlAn, please profile your '
        #           'samples using the same MetaPhlAn version.\n')
        #     return

        iIn = pd.read_csv(f, sep='\t', skiprows=len(headers), names=names, index_col=0)
        profiles_list.append(pd.Series(data=iIn[column], index=iIn.index,
...
argp.add_argument("--column","-c", help="which column where be keep if the tabel have mutiple column")
...
def main( ):
    ...
    merge(args.aistms, open(args.o, 'w') if args.o else sys.stdout, column = args.column)
